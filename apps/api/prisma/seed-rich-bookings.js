...
const generateRichBookingData = async () => {
  ...
  // Create users for each company first
  const users = {};
  ...
  for (const companyCode of companies) {
    ...
    try {
      users[companyCode] = await prisma.user.upsert({
        where: { email: userData.email },
        update: userData,
        create: userData
      });
      console.log(`✅ User created for ${companyCode}`);
    } catch (error) {
      console.log(`⚠️  User already exists for ${companyCode}`);
      users[companyCode] = await prisma.user.findUnique({
        where: { email: userData.email }
      });
    }
  }

  // Generate bookings for each company - 100 per month for 3 months
  let totalBookings = 0;
  ...
  for (const companyCode of companies) {
    ...
    for (let month = 0; month < 3; month++) {
      ...
      const endDate = new Date(2025, month + 2, 0); // End of each month
      
      for (let i = 1; i <= 100; i++) {
        const bookingStartDate = getRandomDate(startDate, endDate);
        const days = Math.floor(Math.random() * 10) + 3; // 3-12 days
        const nights = days - 1;
        const bookingEndDate = new Date(bookingStartDate);
        bookingEndDate.setDate(bookingEndDate.getDate() + days);
        
        const paxCount = Math.floor(Math.random() * 50) + 1; // 1-50 people
        ...
        const bookingData = {
          ...
          depositAmount: Math.floor(totalPrice * 0.3), // 30% deposit
          currency: 'KRW',
          ...
          createdBy: users[companyCode].id,
          updatedBy: users[companyCode].id,
          version: 1
        };
