...
export default function ReservationsPage() {
    ...
    (Array.isArray(bookings) ? bookings : []).forEach((booking: any) => {
      if (booking.startDate || booking.departureDate || booking.date) {
        ...
        bookingsMap[dateStr].push({
          id: booking.id,
          title: booking.customerName || booking.teamName || 'Unknown',
          type: booking.bookingType?.toLowerCase() || 'other',
          status: booking.status?.toLowerCase() || 'pending',
          startDate: booking.startDate || booking.departureDate || booking.date,
          endDate: booking.endDate || booking.returnDate || booking.startDate,
  ...
  const handleExport = (type: 'excel' | 'pdf') => {
    ...
    if (type === 'excel') {
      exportToExcel(exportData as any, 'entrip_bookings');
    } else {
      exportToPDF(exportData as any, 'entrip_bookings');
    }
  }
