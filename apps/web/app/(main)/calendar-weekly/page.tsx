...
import { QuickBookingModal, EditBookingModal, WeeklySummaryFooter } from '@entrip/ui';
...
const convertToBookingEvent = (booking: Booking): BookingEvent => {
  console.log('[CalendarWeeklyPage] Raw booking data:', booking);
  ...
  if (dateObj) {
    try {
      ...
      console.error('[CalendarWeeklyPage] Date formatting error:', e, booking);
    }
  }
  ...
  console.log('[CalendarWeeklyPage] Converted booking:', result);
  return result;
};
...
export default function CalendarWeeklyPage() {
  ...
  const bookings = useMemo(() => {
    console.log('[CalendarWeeklyPage] Computing bookings, apiBookings:', apiBookings?.length || 0);
    console.log('[CalendarWeeklyPage] Current week:', format(currentWeek, 'yyyy-MM-dd'));
    ...
    if (!apiBookings || apiBookings.length === 0) {
      console.log('[CalendarWeeklyPage] No API bookings available');
    ...
    console.log('[CalendarWeeklyPage] Week range:', {
    ...
    const weeklyBookings = apiBookings
      .filter((booking: Booking) => {
        try {
          ...
          if (isInRange) {
            console.log('[CalendarWeeklyPage] Booking in range:', {
              id: booking.id,
          ...
          console.error('[CalendarWeeklyPage] Date filter error:', e, booking);
          return false;
        }
      })
      .map(convertToBookingEvent);
    
    console.log('[CalendarWeeklyPage] Filtered bookings count:', weeklyBookings.length);
    
    weeklyBookings.forEach((booking: BookingEvent) => {
      const dateKey = booking.date;
      
      if (dateKey === 'invalid-date') {
        console.warn('[CalendarWeeklyPage] Invalid date for booking:', booking);
        return;
      }
      ...
      if (!exists) {
        ...
        console.log('[CalendarWeeklyPage] Duplicate booking skipped:', booking.id);
      }
    });
    
    console.log('[CalendarWeeklyPage] Final grouped bookings:', {
    ...
    <>
      ...
      >
        <div className="flex-1 min-h-0 overflow-y-auto">
          <div className="pb-4">
            <WeekView currentDate={currentWeek} />
