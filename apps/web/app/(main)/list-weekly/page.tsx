...
const convertToBookingEvent = (booking: Booking): BookingEvent => {
  console.log('[ListWeeklyPage] Raw booking data:', booking);
  ...
  if (dateObj) {
    try {
      ...
      console.error('[ListWeeklyPage] Date formatting error:', e, booking);
    }
  }
  ...
  console.log('[ListWeeklyPage] Converted booking:', result);
  return result;
};

export default function ListWeeklyPageComponent() {
  ...
  const bookings = useMemo(() => {
    console.log('[ListWeeklyPage] Computing bookings, apiBookings:', apiBookings?.length || 0);
    console.log('[ListWeeklyPage] Current week:', format(currentWeek, 'yyyy-MM-dd'));
    
    let combinedBookings: BookingEvent[] = [...bookingData];
    
    if (!apiBookings || apiBookings.length === 0) {
      console.log('[ListWeeklyPage] No API bookings available');
    ...
    console.log('[ListWeeklyPage] Week range:', {
    ...
    const weeklyBookings = apiBookings
      .filter((booking: Booking) => {
        try {
          ...
          if (isInRange) {
            console.log('[ListWeeklyPage] Booking in range:', {
              id: booking.id,
          ...
          console.error('[ListWeeklyPage] Date filter error:', e, booking);
          return false;
        }
      })
      .map(convertToBookingEvent);
    
    console.log('[ListWeeklyPage] Filtered bookings count:', weeklyBookings.length);
    ...
    weeklyBookings.forEach((booking: BookingEvent) => {
      ...
      if (!exists) {
        ...
        console.log('[ListWeeklyPage] Duplicate booking skipped:', booking.id);
      }
    });
    
    console.log('[ListWeeklyPage] Final bookings count:', combinedBookings.length);
