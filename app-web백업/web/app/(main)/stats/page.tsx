    ...
    const fetchStatsData = async () => {
      try {
        ...
        const [overviewResponse, periodResponse] = await Promise.all([
          axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/stats/overview`),
          axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/stats/period/${selectedPeriod}`)
        ])
        
        setStatsData(overviewResponse.data)
